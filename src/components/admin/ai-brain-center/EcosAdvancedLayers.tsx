      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-white">
        <div>
          <div className="flex items-center gap-2">
            <span className="bg-[#07C2E3] text-black font-extrabold text-[9px] px-1.5 py-0.5 rounded font-mono">L321-330</span>
            <h3 className="font-sans font-bold text-base text-[#07C2E3]">Enterprise Memory Consolidation Engine</h3>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            将散落的企业历史运营细节自动提炼为「经营经验模式（Memory Patterns）」，评估成功率、置信度，计算衰退因子并实现动态正反馈增强。
          </p>
        </div>
        <div className="flex gap-2">
          <span className="bg-slate-800 text-slate-300 text-[10px] font-mono px-2 py-1 rounded">Active Patterns: {memPatterns.length}</span>
        </div>
      </div>

      {/* Grid: 2 columns */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left side: Memory Pattern list */}
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-sans font-bold text-sm text-slate-950 flex items-center gap-2">
                <span className="text-[#07C2E3]">💾</span> Consolidated Memory Patterns
              </h4>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <h5 className="font-bold text-xs truncate max-w-[70%]">{pat.pattern_name}</h5>
                      <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${pat.success_rate >= 80 ? 'bg-emerald-950 text-emerald-405 text-emerald-400' : 'bg-amber-900/30 text-amber-500'}`}>
                        {pat.success_rate}% Success
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {pat.context_tags.map((tag, i) => (
                    ...
                    <div className="flex justify-between items-center text-[10px] text-slate-400 font-mono mt-1">
                      <span>Applications: {pat.total_applications} times</span>
                      <span>Last: {new Date(pat.last_applied_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {/* Manual Consolidation Action Form */}
          <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
            <h4 className="font-sans font-bold text-sm text-slate-955 gap-1 text-slate-900">
              🛠️ Consolidate New Operating Experience
            </h4>
            <p className="text-xs text-slate-505 text-slate-500">
              将近期的关键战役结果固化为系统性经营模式，让多智能体在未来面临类似环境变量时自动继承并调度。
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-bold font-mono mb-1">Pattern Name</label>
                <input
                  type="text"
                  value={newMemPatName}
                  onChange={(e) => setNewMemPatName(e.target.value)}
                  placeholder="e.g. 巴黎高净值羊毛大衣降温狂热"
              ...
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-bold font-mono mb-1">Tags (Comma Sep.)</label>
                <input
                  type="text"
                  value={newMemPatTags}
                  onChange={(e) => setNewMemPatTags(e.target.value)}
                  placeholder="France, Wool, Coat"
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-205 rounded hover:border-slate-350 focus:outline-none focus:border-[#07C2E3]"
                />
              </div>
              <div>
                <label className="block text-[10px] text-slate-400 uppercase font-bold font-mono mb-1">Success Rate (0-100%)</label>
                <input
                  type="number"
                  value={newMemPatSuccess}
                  onChange={(e) => setNewMemPatSuccess(parseInt(e.target.value) || 85)}
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-205 rounded hover:border-slate-350 focus:outline-none focus:border-[#07C2E3]"
                />
              </div>
              <div className="flex items-end">
                <button
                  onClick={() => {
                    if (!newMemPatName.trim()) {
                      triggerSuccess('Error: Pattern name is required.');
                      return;
                    }
                    const created = dbEngine.memory_patterns.create({
                      pattern_name: newMemPatName,
                      context_tags: newMemPatTags.split(',').map(s => s.trim()).filter(Boolean),
                      success_rate: newMemPatSuccess,
                      total_applications: 1,
                      last_applied_at: new Date().toISOString()
                    });
                    // Seed default confidence/decay/weight
                    dbEngine.memory_confidence.create({ pattern_id: created.id, confidence_score: 90, sample_size: 10, std_deviation: 4.5 });
                    dbEngine.memory_decay.create({ pattern_id: created.id, half_life_days: 120, current_decay_factor: 1.0, last_decay_calculated_at: new Date().toISOString() });
                    dbEngine.memory_weights.create({ pattern_id: created.id, factor_name: 'Custom Action Override', weight_modifier: 1.25, is_positive: true });
                    
                    setNewMemPatName('');
                    setTick(t => t + 1);
                    triggerSuccess('Successfully consolidated experience into Global Knowledge Base.');
                  }}
                  className="w-full bg-[#07C2E3] hover:bg-[#06B2D0] active:bg-[#059BBC] text-black font-extrabold text-xs py-2.5 px-4 rounded transition"
                >
                  Add to Memory Brain
                </button>
              </div>
        ...
        {/* Right side: Detailed weights, confidence, decay reinforcement metrics */}
        <div className="lg:col-span-5 space-y-4">
          {(() => {
            const activePattern = memPatterns.find(p => p.id === selectedPatId) || memPatterns[0];
            if (!activePattern) return <div className="text-xs text-slate-400">No pattern selected.</div>;
            
            const weights = memWeights.filter(w => w.pattern_id === activePattern.id);
            const confidence = memConfidence.find(c => c.pattern_id === activePattern.id);
            const decay = memDecay.find(d => d.pattern_id === activePattern.id);
            const reinforcements = memReinforcements.filter(r => r.pattern_id === activePattern.id);

            return (
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl text-slate-100 space-y-4" id="pattern_detailed_twin">
                <div className="border-b border-slate-800 pb-3">
                  <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">Currently Auditing Pattern</span>
                  <h4 className="font-sans font-bold text-sm text-white mt-1">{activePattern.pattern_name}</h4>
                </div>

                {/* 1. Deep Memory Weights modifiers */}
                <div className="space-y-2">
                  <h5 className="text-xs font-bold text-slate-350 flex justify-between">
                    <span>🧬 Logical Association Weights</span>
                    <span className="text-[#07C2E3] font-mono text-[10px]">Multiplier</span>
                  </h5>
                  <div className="space-y-1.5 max-h-[140px] overflow-y-auto">
                    {weights.length === 0 ? (
                      <p className="text-[11px] text-slate-500 italic">No static weights specified. Evaluated using base vector coefficients.</p>
                    ) : (
                      weights.map((w) => (
                        <div key={w.id} className="bg-slate-950 p-2 rounded flex justify-between items-center text-[11px] border border-slate-900">
                          <span className="text-slate-400">{w.factor_name}</span>
                          <span className={`font-mono font-bold ${w.is_positive ? 'text-[#07C2E3]' : 'text-rose-400'}`}>
                            {w.is_positive ? '+' : ''}{w.weight_modifier}x
                          </span>
                        </div>
                      ))
                    )}
                ...
                <div className="bg-slate-950/40 p-3 rounded-lg border border-slate-800/40 space-y-2">
                  <h5 className="text-xs font-bold text-slate-300 flex justify-between">
                    <span>📊 Statistical Confidence Profile</span>
                    <span className="text-emerald-450 font-mono text-[10px]">VERIFIED</span>
                  </h5>
                  {confidence ? (
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-slate-950 p-2 rounded">
                        <span className="block text-[8px] text-slate-500 font-mono">CONFIDENCE</span>
                        <span className="font-mono text-xs font-extrabold text-[#07C2E3]">{confidence.confidence_score}%</span>
                      </div>
                      <div className="bg-slate-950 p-2 rounded">
                        <span className="block text-[8px] text-slate-500 font-mono">SAMPLE SIZE</span>
                        <span className="font-mono text-xs font-extrabold text-white">{confidence.sample_size} runs</span>
                      </div>
                      <div className="bg-slate-950 p-2 rounded">
                        <span className="block text-[8px] text-slate-500 font-mono">STD. DEVIATION</span>
                        <span className="font-mono text-xs font-extrabold text-slate-300">{confidence.std_deviation}σ</span>
                      </div>
                    </div>
                  ) : (
                    <p className="text-[10px] text-slate-550 italic">No confidence calculated.</p>
                  )}
                </div>
                ...
                <div className="space-y-2 bg-slate-950/40 p-3 rounded-lg border border-slate-800/40">
                  <h5 className="text-xs font-bold text-slate-300">
                    ⏳ Temporal Memory Decay & Half-life
                  </h5>
                  {decay ? (
                    <div className="flex justify-between items-center text-[11px]">
                      <div>
                        <span className="text-slate-400">Half-Life Half Life: </span>
                        <span className="text-white font-mono font-bold">{decay.half_life_days} days</span>
                      </div>
                      <div className="bg-slate-955 px-2 py-1 rounded bg-slate-950">
                        <span className="text-slate-550 mr-1.5 uppercase text-[9px]">Retention factor:</span>
                        <span className="font-mono text-emerald-440 font-bold text-emerald-400">{(decay.current_decay_factor * 100).toFixed(0)}%</span>
                      </div>
                    </div>
                  ) : (
                    <p className="text-[10px] text-slate-500 italic">No decay pattern set.</p>
                  )}
                </div>

                {/* 4. Memory Reinforcement Log */}
                <div className="space-y-2">
                  <h5 className="text-xs font-bold text-slate-350">
                    🔄 Reinforcement Event Footprints
                  </h5>
                  <div className="space-y-1.5 max-h-[140px] overflow-y-auto font-mono text-[10px]">
                    {reinforcements.length === 0 ? (
                      <p className="text-slate-500 italic text-[10px]">No reinforcement events registered.</p>
                    ) : (
                      reinforcements.map((rf) => (
                        <div key={rf.id} className="bg-slate-950 p-2 rounded flex justify-between items-center text-slate-300 border border-slate-900">
                          <div>
                            <span className="block text-white max-w-[200px] truncate">{rf.reinforcement_event}</span>
                            <span className="text-[8px] text-slate-505">{new Date(rf.logged_at).toLocaleTimeString()}</span>
                          </div>
                          <span className="text-emerald-440 font-bold bg-emerald-950/40 border border-emerald-900 px-1 rounded text-emerald-400">
                            +{rf.adjustment_delta}% Strength
                          </span>
                        </div>
                      ))
                    )}
                ...
                <button
                  onClick={() => {
                    dbEngine.memory_reinforcement.create({
                      pattern_id: activePattern.id,
                      reinforcement_event: `Manual Super Admin Calibration Check: ${new Date().toLocaleDateString()}`,
                      adjustment_delta: 3,
                      logged_at: new Date().toISOString()
                    });
                    triggerSuccess('Reinforcement footprint injected. Enhanced associated weight matrices.');
                    setTick(t => t + 1);
                  }}
                  className="w-full bg-slate-800 hover:bg-slate-700 font-extrabold text-xs text-[#07C2E3] py-2.5 rounded border border-slate-700 transition"
                >
                  Execute Memory Reinforcement (+3%)
                </button>
              </div>
            );
          })()}
